...
public class DiskWorker // extends SwingWorker<Boolean, DiskMark>
 {
     CustomUserInterface type;
public DiskWorker(CustomUserInterface customUserInterface){
    this.type=customUserInterface;
}
    ...
    protected Boolean doInBackground() throws Exception {
        ...
        if (App.writeTest) {
             ...
             */
            for (int m = startFileNum; m < startFileNum + App.numOfMarks && !type.CustIsCanceled(); m++) {
                ...
                try {
                    try (RandomAccessFile rAccFile = new RandomAccessFile(testFile, mode)) {
                        for (int b = 0; b < numOfBlocks; b++) {
                             ...
                             */
                            type.CustSetProgress((int) percentComplete);
                        }
                    }
        ...
        if (App.readTest && App.writeTest && !type.CustIsCanceled()) {
    ...
    protected void done() {
     ...
     public boolean CustIsCanceled() {
         return type.CustIsCanceled();
     }



     public void CustSetProgress(int x) {
         type.CustSetProgress(x);
     }
     ...
     public Boolean CustCancel(boolean mayInterruptIfRunning) {
         return type.CustCancel(mayInterruptIfRunning);
     }

     public void CustAddPropertyChangeListener(PropertyChangeListener listener) {
         type.CustAddPropertyChangeListener(listener);
     }
